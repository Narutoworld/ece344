...
struct thread *
pid_get_thread(pid_t pid){
	return array_getguy(thread_array, pid);
}

int
get_array_size(){
	return array_getnum(thread_array);
}

void invalid_exit_thread(pid_t pid){
	array_setguy(thread_array,pid, NULL);
	return;
}
/*
...
thread_create(const char *name)
{
	...
	thread->thread_lock = lock_create("");
	
	lock_acquire(thread->thread_lock);

	#if DEBUG_THREAD & DEBUG_CODE
    kprintf("\nthread_create: thread_array:%p, thread:%p\n",thread_array, thread);
    #endif

	array_add(thread_array,thread);
	#if DEBUG_THREAD & DEBUG_CODE
    kprintf("\nthread_create: getnum:%d\n",array_getnum(thread_array));
    #endif
	//assert(array_getnum(thread_array) == curthread->pid);

	thread->pid = array_getnum(thread_array) - 1;
	
	if(thread->pid == 1){
		thread->parent_pid = 0;
	}
	#if DEBUG_CODE & DEBUG_THREAD
...
thread_destroy(struct thread *thread)
{
	...
	kfree(thread->t_name);
	array_setguy(thread_array,thread->pid, NULL);
	#if DEBUG_CODE & DEBUG_THREAD
	kprintf("destroyed process, PID: %d\n", thread->pid);
	#endif
	kfree(thread);
}
...
thread_bootstrap(void)
{
	...
	if (thread_array == NULL){
		panic("Cannot create threads array\n");
	}
	array_add(thread_array, NULL);
	assert(array_getnum(thread_array) == 1);
...
thread_exit(void)
{
	...
	lock_release(curthread->thread_lock);
