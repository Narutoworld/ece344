...
common_prog(int nargs, char **args)
{
	...
	result = thread_fork(args[0] /* thread name */,
			...
			cmd_progthread, &ret_thread);
	if (result) {
		kprintf("thread_fork failed: %s\n", strerror(result));
		return result;
	}
	tf.tf_a0 = ret_thread->pid;
	tf.tf_a1 = 0x010;
	sys_waitpid(&tf, NULL);
...
static
int
cmd_debugflags(int nargs,char **args)
{
    (void)args;
    if (nargs != 1){
...
static
int
switch_debug_info(int nargs,char **args){
    if (nargs != 3){
            kprintf("Usage: df nr on/off\n");
            return EINVAL;
    }
    
    int nr= char_to_int(args[1]);
    if (!(nr >=1 && nr <= 12)){
            kprintf("Usage: df nr on/off\n");
            return EINVAL;
    }
    ...
    if (!strcmp("on",args[2])){
        int increment = get_Debug_Info(nr);
        dbflags = dbflags + increment;
    }
    else if(!strcmp("off",args[2])){
        int decrement = get_Debug_Info(nr);
        dbflags = dbflags - decrement;    
    } 
    else {
        kprintf("the argument is neither on nor off\n");
    }  
    return 0;
}
/*
